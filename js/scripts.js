  ...
  _createClass(ShowMore, [{
    ...
    value: function init() {
      ...
      var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.state,
          btn = _ref.btn,
          modal = _ref.modal,
          productsContainer = _ref.productsContainer;
    ...
    value: function handleClick(event) {
      var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.state,
          btn = _ref2.btn;
    ...
    key: "setModal",
    value: function setModal(btn) {
      var image = btn.querySelector("img"),
          img = image ? image.src : btn.querySelector("div").style.backgroundImage.match(/"((?:\\.|[^"\\])*)"/);
      this.state.modal.querySelector("#show-more-modal--cover").style.backgroundImage = "url(".concat(img, ")");
    }
  }, {
    key: "setProducts",
    value: function setProducts(currentProduct) {
      var _this2 = this;

      document.querySelectorAll("#related_products_box_".concat(currentProduct, " .article")).forEach(function (el) {
        var article = el.cloneNode(true);

        _this2.state.productsContainer.appendChild(article);

        article.addEventListener('mouseenter', function () {
          return article.classList.add("open", "active");
        });
        article.addEventListener('mouseleave', function () {
          return article.classList.remove("open", "active");
        });
      });
    }
  }]);
...
var setupShowMoreBtns = function setupShowMoreBtns() {
  return document.querySelectorAll('a.show-more').forEach(function (btn) {
    return new ShowMore(btn);
  });
};
"use strict";
...
var newsletterCTA = {
  setup: function setup() {
    ...
    var visited = ((_document$cookie$matc = document.cookie.match('(^|;)\\s*visited_wintersphere\\s*=\\s*([^;]+)')) === null || _document$cookie$matc === void 0 ? void 0 : _document$cookie$matc.pop()) || null;
    if (!!visited) _this.modalCountdown = setTimeout(_this.modalPopUp, 300);
    newsletterCTA.handleModalShow();
    newsletterCTA.checkPolicy();
  },
  handleModalShow: function handleModalShow() {
    $("#newsletter_modal").on("show.bs.modal hidden.bs.modal", function (e) {
      clearTimeout(_this.modalCountdown);
      $("#newsletter-box").detach().appendTo(e.type !== "show.bs.modal" ? "#newsletter_modal_content" : "#newsletter-box--wrapper");
      $.cookie('visited_wintersphere', 'yes', {
        expires: 1,
        path: '/'
      });
    });
  ...
  checkPolicy: function checkPolicy() {
    $('#custom_fields_privacy').on('click touch', function () {
      return $('#newsletter_form_submit').attr('disabled', function (_, attr) {
        return !attr;
      });
    });
  },
  modalPopUp: function modalPopUp() {
    return $("#newsletter_modal").modal("show");
  }
};
"use strict";
...
var VideoCTA = /*#__PURE__*/function () {
  function VideoCTA(selector) {
    _classCallCheck(this, VideoCTA);

    this.cta = document.querySelector("[data-play-video=\"".concat(selector, "\"]"));
    this.video = document.querySelector(selector);
    this.init();
  }

  _createClass(VideoCTA, [{
    key: "init",
    value: function init() {
      var _this = this;

      this.cta.addEventListener("click", function () {
        return _this.initHeroVideo();
      });
      this.cta.addEventListener("touch", function () {
        return _this.initHeroVideo();
      });
    }
  }, {
    key: "initHeroVideo",
    value: function initHeroVideo() {
  ...
  _createClass(BackToTopButton, [{
    ...
    value: function handleScroll() {
      var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.state,
          btn = _ref.btn,
          offset = _ref.offset,
          offset_opacity = _ref.offset_opacity;

      window.addEventListener('scroll', function (e) {
        var scrollTop = $(e.target).scrollTop();
        scrollTop > offset ? btn.classList.add("cd-is-visible") : btn.classList.remove("cd-is-visible", "cd-fade-out");
    ...
    value: function handleClick() {
      var _ref2 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.state,
          btn = _ref2.btn,
          scroll_top_duration = _ref2.scroll_top_duration;
...
var scrollMagicWintersphere = {
  ...
  desktopOnlyScenes: function desktopOnlyScenes() {
    if ($(window).width() >= 992) {
      this.parallax();
      this.follow();
      this.video();
  ...
  localize: function localize() {
    document.querySelectorAll("[data-text]").forEach(function (el) {
      return el.innerHTML = text[$(el).data("text")];
    });
    document.querySelectorAll("article").forEach(function (el) {
      var _products$find = products.find(function (item) {
        return item.id === el.id.replace("product_", "");
      }),
          props = _products$find.props,
          name = _products$find.name;

      el.querySelector("h5").innerText = name;
      el.querySelectorAll("li").forEach(function (li, i) {
        return li.innerHTML = "<span>-</span> " + props[i];
      });
    });
  },
  universalScenes: function universalScenes() {
  ...
  video: function video() {
    document.querySelectorAll("video").forEach(function (el, i) {
  ...
  fadeTextIn: function fadeTextIn() {
    return document.querySelectorAll("[data-text], span.fade").forEach(function (el) {
  ...
  resizeToggle: function resizeToggle() {
    $(window).on("load resize", function () {
      ...
      } else if ($(window).width() <= 991 && prevWidth >= $(window).width()) {
        controller.destroy();
        controller = null;
        scrollMagicWintersphere.universalScenes();
  ...
  parallax: function parallax() {
    return document.querySelectorAll(".parallax").forEach(function (el) {
      return new ScrollMagic.Scene({
  ...
  follow: function follow() {
    return document.querySelectorAll(".follow-me").forEach(function (el) {
      return new ScrollMagic.Scene({
        triggerElement: document.querySelector(el.dataset.parent),
  ...
  slideIn: function slideIn() {
    return document.querySelectorAll(".collections-imgs, .slidable").forEach(function (el) {
      new ScrollMagic.Scene({
        triggerElement: el
      }).on("enter", function () {
        return el.classList.add("slideIn");
      }).addTo(controller);
    });
  }
};
